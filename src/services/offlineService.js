...
class OfflineService {
  ...
  async storeBranche(nomBranche, entrepriseId) {
    ...
    try {
      ...
      console.error(
        "Erreur lors de l'ajout de la branche dans IndexedDB:",
        error
      );
  ...
  async ChangeEtat(uuidProspectToUpdate, nouveauStatut, nouveauSyncState) {
    try {
      await AppStorage.updateProspectChange(
  ...
  async changeProspect(
    ...
    await AppStorage.updateProspectEtat(
      uuidProspectToUpdate,
      newState,
      newSyncState
    );
  ...
  async deleteProspect(uuidProspectToUpdate, newDelete, newSyncState) {
    try {
      await AppStorage.updateProspectDelete(
        uuidProspectToUpdate,
        newDelete,
        newSyncState
      );
  ...
  async addProspectBranche(form, userId, entrepriseId, uuidProspect) {
    ...
    // Une fois que la mise à jour est effectuée avec succès, récupérez la liste mise à jour des prospects
    const updatedBrancheProspects =
      await AppStorage.getBrancheProspectsByuuidProspect(uuidProspect);
  ...
  async updateAvenantPayerApporteur(
    ...
    const avenantMisAJour = await AppStorage.updateAvenantPayerApporteur(
      uuidAvenantToUpdate,
      newPayer,
      newSyncState
    );
    ...
    // Une fois que la mise à jour est effectuée avec succès, récupérez la liste mise à jour des prospects
    const updatedAvenants = await AppStorage.getSommeCommissionsApporteurPayer(
      uuidApporteur
    );
  ...
  async updateTauxApporteur(uuidTauxApporteurUpdate, newTaux, newSyncState) {
    await AppStorage.updateTauxApporteurByUuid(
    ...
    // Une fois que la mise à jour est effectuée avec succès, récupérez la liste mise à jour des prospects
    const updatedTauxApporteurs =
      await AppStorage.getTauxApporteursByIdApporteur(uuidTauxApporteurUpdate);
  ...
  async updateApporteur(apporteurtoedit, uuidApporteurToUpdate) {
    const nouvellesInfos = {
      ...
      code_postal: apporteurtoedit.code_postal,
      contact_apporteur: apporteurtoedit.contact_apporteur,
      sync: 0,
    };
  ...
  async updateAvenantPayerCompagnie(
    ...
    const avenantMisAJour = await AppStorage.updateAvenantPayerCompagnie(
      uuidAvenantToUpdate,
      newPayer,
      newSyncState
    );
    ...
    // // Une fois que la mise à jour est effectuée avec succès, récupérez la liste mise à jour des prospects
    const updatedAvenants = await AppStorage.getSommeCommissionsCompagniePayer(
      uuidApporteur
    );
  ...
  async storeCompagnie(form, userId, entrepriseId, unique, donnees, datas) {
    ...
    for (const tauxCompagnie of tauxCompagnies) {
      await AppStorage.storeDataInIndexedDB("tauxcompagnies", [tauxCompagnie]);
    }
  }
  ...
  async storeContrat(
    ...
    const uuid = uuidv4();
    const clientName = await AppStorage.getClientNameByUUID(form.client_id);
    const clientCode = await AppStorage.getClientCodeByUUID(form.client_id);
  ...
  async getTauxParIdBrancheEtApporteur(id_branche, optional) {
    return await AppStorage.getTauxParIdBrancheEtApporteur(
      id_branche,
      optional
    );
  }
  ...
  async updateContrat(
    ...
    const udpatedContrat = await AppStorage.updateContrat(
      uuidContratToUpdate,
      nouvellesInfos
    );
  ...
  async updateAvenantSolde(
    ...
    const avenantMisAJour = await AppStorage.updateAvenantSolde(
      uuidAvenantToUpdate,
      newSolde,
      newSyncState
    );
    ...
    // Une fois que la mise à jour est effectuée avec succès, récupérez la liste mise à jour des prospects
    const updatedAvenants = await AppStorage.getAvenantsByUuidContrat(
      uuidContrat
    );
  ...
  async updateAvenantReverse(
    ...
    const avenantMisAJour = await AppStorage.updateAvenantReverse(
      uuidAvenantToUpdate,
      newReverse,
      newSyncState
    );
  ...
  async storeSinistre(form, userId, entrepriseId, police) {
    ...
    // Enregistré les contrats dans IndexedDB
    const sinistres = await AppStorage.storeDataInIndexedDB(
      "sinistres",
      newSinistreData
    );
  ...
  async updateSinistre(uuidSinistreToUpdate, sinistres) {
    ...
    const udpatedSinistre = await AppStorage.updateSinistre(
